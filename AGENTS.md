...
> Svalboard, `keyboards/svalboard/keymaps/flask/`, with its own AGENTS.md and
...
> - **`.Codex/skills/madromys`** — retired, kept next to the Adept history it
>   documents.
> - **`.Codex/skills/hid-remapper-vial`** — belongs to `~/hid-remapper`
>   (AlooMapper), not to either QMK tree. Misfiled here; move it if it starts
>   mattering.
> - **`.Codex/handoffs/`** — the Adept and NLKB16 history.
>
> If the ask is about the Svalboard, the shared modules, or either Flask client,
> work in `~/svalboard-vial-qmk` and read its
> `.Codex/skills/flask-ecosystem/SKILL.md`.
...
> `~/ZMK-Flask`** with its own AGENTS.md and memory. If the user asks about
